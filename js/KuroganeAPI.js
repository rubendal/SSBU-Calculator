...
class MoveParser {
	constructor(id, name, base_damage, angle, bkb, kbg, hitboxActive, faf, landingLag, autoCancel, weightDependent, ignore_hitboxes, hitboxActiveTooltip, baseDamageTooltip) {
		...
		if (hitboxActiveTooltip || baseDamageTooltip) {
			...
			var addHitstun = 0;
			if (hitboxActiveTooltip != null && hitboxActiveTooltip.AdditionalHitstun != undefined && hitboxActiveTooltip.AdditionalHitstun != "") {
				addHitstun = parseFloat(hitboxActiveTooltip.AdditionalHitstun);
			} // addHitstun not available from API

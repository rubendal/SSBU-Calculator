...
class MoveParser {
	constructor(id, name, base_damage, angle, bkb, kbg, hitboxActive, faf, landingLag, autoCancel, weightDependent, ignore_hitboxes, hitboxActiveTooltip, baseDamageTooltip) {
		...
		if (hitboxActiveTooltip || baseDamageTooltip) {
			...
			var shieldDamage = 0;
			if (hitboxActiveTooltip.SD != undefined && hitboxActiveTooltip.SD != "") {
				shieldDamage = parseFloat(hitboxActiveTooltip.SD);
			} else if (baseDamageTooltip.SD != undefined && baseDamageTooltip.SD != "") {
			...
			var setweight = false;
			if (hitboxActiveTooltip.SetWeight != undefined) {
				shieldDamage = hitboxActiveTooltip.SetWeight;
			} else if (baseDamageTooltip.SetWeight != undefined) {
				setweight = baseDamageTooltip.SetWeight;
			}

			this.setweight = setweight;

			var shieldstunMult = 0;
			if (hitboxActiveTooltip.ShieldstunMultiplier != undefined && hitboxActiveTooltip.ShieldstunMultiplier != "") {
				shieldstunMult = parseFloat(hitboxActiveTooltip.ShieldstunMultiplier);
			} else if (baseDamageTooltip.ShieldstunMultiplier != undefined && baseDamageTooltip.ShieldstunMultiplier != "") {
				shieldstunMult = parseFloat(baseDamageTooltip.ShieldstunMultiplier);
			}
...
function getMoveset(attacker, $scope) {
	...
	//			var move = moveset[i];
	//			var parser = new MoveParser(move.InstanceId, move.Name, move.BaseDamage, move.Angle, move.BaseKnockBackSetKnockback, move.KnockbackGrowth, move.HitboxActive, move.FirstActionableFrame, move.LandingLag, move.AutoCancel, move.IsWeightDependent, false, move.HitboxActiveTooltip, move.BaseDamageTooltip);
		...
		if (character != null) {
			...
			loadAsyncFunctionJSON("https://test-khapi.frannsoft.com/api/characters/" + id + "/moves?expand=true&game=ultimate", function (moveset) {
				if (moveset != null) {
					...
					for (var i = 0; i < moveset.length; i++) {
						var move = moveset[i];
						var parser = new MoveParser(move.InstanceId, move.Name, move.BaseDamage.Normal, move.Angle, move.BaseKnockBackSetKnockback, move.KnockbackGrowth, move.HitboxActive.Frames, move.FirstActionableFrame, move.LandingLag, move.AutoCancel, move.IsWeightDependent, false, move.HitboxActive, move.BaseDamage);
						for (var c = 0; c < parser.moves.length; c++) {
							var m = parser.moves[c];
							m.id = count;
							if (!m.grab && m.valid) {
			...
			//Character isn't on API in Ultimate, check Smash 4 data

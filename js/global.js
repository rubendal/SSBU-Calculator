...
class Character {
    constructor(n) {
		...
		if (newCharacters.indexOf(this.display_name) == -1) {
			this.attributes = loadJSON(this.name);
		}
		else if (this.name == "Daisy") {
			this.attributes = loadJSON("Peach");
		}
		else {
			this.attributes = loadJSON("Cloud");
		}
...
function getTitle(attribute) {
    var titles = [
		...
		{ "attribute": "1v1 Damage increase", "title": "Damage is increased by 20% on 1v1 battles on SSBU, this damage increase works like a damage dealt/taken multiplier which doesn't affect move's base damage on the KB calculation" },
